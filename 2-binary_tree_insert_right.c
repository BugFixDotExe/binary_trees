...
binary_tree_t *binary_tree_insert_right(binary_tree_t *parent, int value)
{
	...
	if (parent == NULL)
		return (NULL);
	/**
	 * A situation might arise or edge case where the right of the root or parent
	 * is NULL, indicating the parent or root node has no child or leaf node
	 * Thus the following has to be done:
	 * Create a new child or leaf node for the parent or root node
	...
	{
		new_node = binary_tree_node(parent->right->parent, value);
